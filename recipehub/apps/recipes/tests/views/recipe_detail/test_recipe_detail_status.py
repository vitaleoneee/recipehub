...
class TestRecipeDetailStatus:
    """Tests for recipe detail status codes and template rendering"""

    def test_recipe_detail_status_code(self, client, users_list):
        ...
        unapproved_recipe = RecipeFactory.create(
            user=recipe_owner_user, slug="pizza", moderation_status="rejected"
        )

        # Testing redirection to login for anonymous users
        redirection_response = client.get(
        ...
        bad_response = client.get(
            reverse("recipes:recipe-detail", kwargs={"slug": unapproved_recipe.slug})
        )
