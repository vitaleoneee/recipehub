...
def test_recipe_list_view_with_pagination(client, users_list):
    ...
    RecipeFactory.create(user=users_list["recipe_owner_user"], slug="fish-1", approved=True)
    RecipeFactory.create(user=users_list["first_simple_user"], slug="fish-2", approved=True)
    RecipeFactory.create(user=users_list["second_simple_user"], slug="fish-3", approved=True)
    RecipeFactory.create(user=users_list["recipe_owner_user"], slug="fish-4", approved=True)
    RecipeFactory.create(user=users_list["first_simple_user"], slug="pizza", approved=False)
