...
def test_recipe_list_view_with_pagination(client, users_list):
    ...
    first_fish_recipe = RecipeFactory.create(user=users_list["recipe_owner_user"], name="Fish", slug="fish",
                                             approved=True)
    second_fish_recipe = RecipeFactory.create(user=users_list["first_simple_user"], name="Fish", slug="fresh-fish",
                                              ingredients="tomato - 1ks",
                                              approved=True)
    vareniki_recipe = RecipeFactory.create(user=users_list["second_simple_user"], recipe_text="Bake the dough",
                                           slug="vareniki",
                                           approved=True)
