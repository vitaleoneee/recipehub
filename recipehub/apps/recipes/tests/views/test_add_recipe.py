...
class TestRecipeFormValidation:
    """Validation tests for RecipeForm"""
...
@pytest.mark.django_db
class TestRecipeAddView:
    """Tests for adding recipes, photos and form errors"""

    def test_add_recipe_with_photo(self, client, users_list):
    ...
    def test_add_recipe_with_non_image_as_photo(self, client, users_list):
        user = users_list["first_simple_user"]
        client.force_login(user)
        category = CategoryFactory()
    ...
    def test_recipe_form_without_required_fields(self, category_id, name, servings):
        if category_id == 1:
    ...
    def test_add_recipe_template(self, client, users_list):
    ...
    def test_add_recipe_clean_ingredients(
        self, ingredients, should_be_valid, expected_error
    ):
