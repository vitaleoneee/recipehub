...
class TestRecipeBuilder:
    """Tests for recipe builder views and session handling"""

    def test_recipe_builder_template(self, client, users_list):
        """Unauthenticated users are redirected; authenticated users see template"""
    ...
    def test_recipe_builder_add_ingredients(self, client, users_list):
        """Adding ingredients stores them in session and context"""
        client.force_login(users_list["first_simple_user"])
    ...
    def test_recipe_builder_add_ingredients_with_no_data(self, client, users_list):
        """Posting empty ingredients shows message and clears session"""
        client.force_login(users_list["first_simple_user"])
        response = client.post(
            reverse("recipes:recipe-builder"), data={"ingredients": ""}
        )
        assert response.status_code == 200
    ...
    def test_recipe_builder_clear_ingredients(self, client, users_list):
        """Clear ingredients removes them from session and context"""
        client.force_login(users_list["first_simple_user"])
        client.post(
            reverse("recipes:recipe-builder"), data={"ingredients": "tea, cheeps"}
        )
    ...
    def test_recipe_builder_find_recipes(self, client, users_list):
        """Searching by ingredients filters recipes"""
        client.force_login(users_list["first_simple_user"])
